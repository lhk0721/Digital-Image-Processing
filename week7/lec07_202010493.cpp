...
BYTE Median(BYTE*arr, int size){
    const int S = size;
    for(int i=0; i<size-1; i++){//pivotIndex
...
int main()
{
	...
	// int Margin = Length / 2; // 마스크 중심에서 가장자리까지의 거리
	// int WSize = Length * Length; // 마스크 크기(마스크 내 픽셀 갯수)
	// BYTE* temp = (BYTE*)malloc(sizeof(BYTE) * WSize); //정렬을 위한 임시 배열
	// int W = hInfo.biWidth, H = hInfo.biHeight; // 이미지 사이즈
	// int i, j, m, n; // 출력을 위한 for문 외부 선언. 
	// for (i = Margin; i < H - Margin; i++) { //외부 2중 루프-픽셀 순회
	// 	for (j = Margin; j < W - Margin; j++) { 
	// 		for (m = -Margin; m <= Margin; m++) { // 내부 2중 루프 - 마스크 수집
